...
def build_model(input_shape, use_mann_layers = False):
    ...
    if use_mann_layers:
        x = mann.layers.MaskedConv2D(8, activation = 'relu')(input_layer)
        x = mann.layers.MaskedConv2D(8, activation = 'relu')(x)
        x = tf.keras.layers.MaxPool2D(strides = 1)(x)
        x = mann.layers.MaskedConv2D(16, activation = 'relu')(x)
        x = mann.layers.MaskedConv2D(16, activation = 'relu')(x)
        x = tf.keras.layers.MaxPool2D(strides = 1)(x)
        x = tf.keras.layers.Flatten()(x)
        x = mann.layers.MaskedDense(32, activation = 'relu')(x)
